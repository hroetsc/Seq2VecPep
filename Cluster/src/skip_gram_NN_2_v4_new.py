...
BATCH_SIZE = 64 * hvd.size()

print('per worker batch size: ', BATCH_SIZE)

### output - this is not ideal ...
checkpoints = '/scratch2/hroetsc/Seq2Vec/results/hp_v50k_model_w5_d128/ckpts'
weights_path = '/scratch2/hroetsc/Seq2Vec/results/hp_v50k_model_w5_d128/weights.h5'
model_path = '/scratch2/hroetsc/Seq2Vec/results/hp_v50k_model_w5_d128/model.h5'
embedding_layer = '/scratch2/hroetsc/Seq2Vec/results/hp_v50k_w5_d128_embedding.csv'
...
def build_and_compile_model():
    ...
    dot_product = layers.dot([target, context], axes=2, normalize=True, name='dot_product')
    ...
    opt = keras.optimizers.Adam(learning_rate=0.01 * hvd.size(),
                                name='Adam')
...
# preprocessing: remove ambigous skipgrams
skip_grams.columns = ['target', 'context', 'label']
skip_grams = skip_grams.drop_duplicates(keep='first', subset=['target', 'context'])

# split skip-grams into target, context and label np.array()
target_word = np.array(skip_grams['target'], dtype='int32')
context_word = np.array(skip_grams['context'], dtype='int32')
Y = np.array(skip_grams['label'], dtype='int32')

### tmp!
#ind = np.random.randint(0, target_word.shape[0], int(5e04))
...
target_train, target_test, context_train, context_test, Y_train, Y_test = train_test_split(target_word, context_word, Y,
                                                                                           test_size=valSplit)
...
fit = model.fit(BatchGenerator(target_train, context_train, Y_train, BATCH_SIZE),
                validation_data=BatchGenerator(target_test, context_test, Y_test, BATCH_SIZE),
