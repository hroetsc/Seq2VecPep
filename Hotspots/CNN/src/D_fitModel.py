...
def residual_block(prev_layer, no_filters):
    conv = layers.Conv1D(no_filters, 3, 2,
                         padding='same',
                         kernel_regularizer=keras.regularizers.l2(l=0.01),
...
def build_and_compile_model():
    ...
    out_norm = layers.BatchNormalization(trainable=True)(dense4)
    out = layers.Dense(1, activation='relu',
                       kernel_initializer=tf.keras.initializers.HeNormal(),
    ...
    tf.print('number of convolutions: 4')
    tf.print('number of dense layers: 5')
