...
input = snakemake@input[["sequence_repres"]]

foreach(i = 1:length(input)) %dopar% {
  ### INPUT ###
  emb = read.csv(snakemake@input[["sequence_repres"]][i],
                 stringsAsFactors = F, header = T)
  
  ### MAIN PART ###
  emb = emb[order(emb$Accession), ]
  proteins = emb$Accession
  if("gene" %in% colnames(emb)){
    genes = emb$gene
  }
  ...
  # add protein/gene information
  if("gene" %in% colnames(emb)){
    genes = emb$gene
    res = matrix(ncol = ncol(sim)+2, nrow = nrow(sim))
    res[, 1] = proteins
    res[, 2] = genes
    res[, c(2:ncol(res))] = sim
    colnames(res) = c("Accession", "gene", seq(1, ncol(sim)))
  } else{
