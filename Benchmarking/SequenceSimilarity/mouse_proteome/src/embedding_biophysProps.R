...
### MAIN PART ###
# clean sequences
sequences$seqs = as.character(sequences$seqs)
a = sapply(toupper(sequences$seqs), protcheck)
names(a) = NULL
print(paste0("found ",length(which(a==F)) , " sequences that are failing the protcheck() and is removing them"))
sequences = sequences[which(a == T), ]
...
# actually calculate
PropMatrix = PepSummary(sequences$seqs)
PropMatrix = cbind(sequences[, c("Accession", "seqs")], PropMatrix)
