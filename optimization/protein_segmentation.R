...
# load the model
bpeModel = bpe_load_model("results/encoded_proteome/opt_BPE_model_100000.bpe",
                          threads = 12)
ModelVocab = bpeModel$vocabulary
ModelVocab = tibble::as_tibble(ModelVocab)

# for hyperparameter optimization, only take a random subset of 10000 proteins
proteins = proteins[sample.int(10000),] #10000
...
### OUTPUT ###
# save words
write.csv(words, file = "results/encoded_proteome/opt_words_100000.csv", row.names = F)
